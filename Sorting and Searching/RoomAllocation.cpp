...
fr(0,n){
if(rooms.size()==0){
    roomAssigned[v[i].second]=roomNo;
    rooms.push({v[i].first.second,roomNo});
    roomNo++;
}else{
if(rooms.top().first<v[i].first.first){
        roomAssigned[v[i].second]=rooms.top().second;
    pair<int,int> x;
    x={v[i].first.second,rooms.top().second};
rooms.pop();
rooms.push(x);
